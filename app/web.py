...
async def history(limit: int = 30):
    """历史记录列表"""
    try:
        from app.persistence import list_history
        return JSONResponse(content={"history": list_history(limit)})
    except Exception as e:
        return JSONResponse(content={"error": str(e)}, status_code=500)


@app.get("/api/history/{record_id}", response_class=JSONResponse)
async def history_detail(record_id: int):
    """加载历史记录"""
    try:
        from app.persistence import get_analysis
...
@app.get("/api/favorites", response_class=JSONResponse)
...
@app.post("/api/favorites", response_class=JSONResponse)
async def favorites_add(code: str = Form(...), name: str = Form(""), note: str = Form("")):
    try:
        from app.persistence import add_favorite
...
@app.delete("/api/favorites/{code}", response_class=JSONResponse)
async def favorites_remove(code: str):
    try:
        from app.persistence import remove_favorite
